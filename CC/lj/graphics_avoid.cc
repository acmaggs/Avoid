...
Graphics::draw(std::vector<double> &p, double FPS,int l, int act){
  ...
  if( XCheckWindowEvent(dsp,da, DestroyNotify , &event)){ XCloseDisplay(dsp); cout<<"X11 problem\n" ;exit(1);}
...
//  cairo_set_source_rgb(cr, 0.0, 0.1, .1);//dark green background
  cairo_set_source_rgb(cr, 1.0, 1, 1);//dark green background
...
//    cairo_arc(cr,   (alpha + gamma* ( i  + lmin)) , 220-1*p[i] , 3.5*alpha, 0, 2 * M_PI);
    cairo_arc(cr,   alpha + (p[i]-lmin)*gamma, 220 , 3.5*alpha, 0, 2 * M_PI);
...
Graphics::Graphics(int N, int Pix, double dmin, double dmax,double diameter){
    ...
    if (( dsp = XOpenDisplay(NULL)) == NULL)      {cout<<"No X11 display\n"; exit(1);}//window management X11, and cairo graphics
...
Graphics::~Graphics(){cairo_destroy (cr);cairo_surface_destroy (sfc); } //clean up function
