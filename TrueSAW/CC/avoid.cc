...
void build_cholesky(vector<double> &d, vector<double> &o, vector<double> &c) { // cholesky factor for 1d laplacian
  int n = d.size();
  for (int i = 0; i < n - 1; i++) {
  ...
  for (int i = 0; i < n; i++)
    c[i] = -1. / sqrt((i + 1) * (i + 2)); // correction for periodic bc
  c[n - 2] = -sqrt(n / (n - 1.));
  c[n - 1] = d[n - 1];
  o[n - 2] = c[n - 2];
}

void random_config(vector<double> &conf, vector<double> &d, vector<double> &o, vector<double> &c,
                   int test) { // solve for a configuration by back substitution
  int n = d.size();
  vector<double> r(n);
  if (!test) {
    for (int i = 0; i < n; i++)
  ...
  for (int i = 0; i < n - 2; i++)
    conf[n - 3 - i] = (r[n - 3 - i] - conf[n - 2 - i] * o[n - 3 - i] - c[n - 3 - i] * conf[n - 1]) / d[n - 3 - i];
}
...
double step(vector<double> &x, vector<int> &count, int &a, int &b) { // single ECMC step
  int n = x.size();
...
void sweep(vector<double> &x, vector<int> &count, int &a, int &b) { // N ECMC steps
...
void build_harmonic(vector<double> &x) {
  int n = x.size();
...
void chain(vector<double> &x, vector<int> &count, int &a, int &b) {
  double t_chain = 0;
...
void mc(vector<double> &x, vector<int> &master, list<int> &lst, list<int> &blst, list<int> &zlst, list<double> &xlst, ofstream &log_out) {
  ...
  for (int l = 0; l < Param::iter; l++) { // main loop for data
    ...
    if (Param::cold) {
      ...
      if (Param::GRAPH)
        for (int i = 0; i < Param::N; i++)
          x[i] = 60 * cos((2. * i * M_PI) / Param::N) - 20;
    } else {
      build_harmonic(x);
    }
