...
inline double root_right(double r, int s) {
...
/** Solution of LambertW for first or second nearest neigbour */
inline double root_left(double r, int s) {
  double pbar = s == 1 ? Lambert::pbar : Lambert::pbar2;
  double p = s == 1 ? Lambert::p : Lambert::p2;
  if (fabs(p) < 1.e-12) {
    return 1.e9;
...
pair<double, double> virial(vector<double> &x, int s) {
  int n = x.size();
  double v = 0;
  double force = 0;
  for (int i = 0; i < n; i++) {
 ...
 */
double expon_step(vector<double> &x, vector<int> &count, long int &a, long int &b,
 ...
 */
void exp_mc(vector<double> &x, vector<int> &master, list<int> &lst, list<int> &blst, list<int> &zlst, list<double> &xlst,
            ofstream &log_out) {
...
void init() { /** Rescale parameters for solution of Lambert equation */
              //  assert(Lambert::p2>0);
              // assert(Lambert::p>0);
  if (fabs(Lambert::p) < 1.e-12)
    Lambert::p = 1.e-14;
  if (fabs(Lambert::p2) < 1.e-12)
    Lambert::p2 = 1.e-14;
  cout << red;
...
void long_run(vector<double> &x) {
  ...
  for (int i = 0; i < Control::sweeps; i++) {
    ...
    if (i > Control::sweeps / 8) {
      ...
      if (i % 1024 == 0)
        cout << "i " << i / 1024 << "/" << Control::sweeps / 1024 << endl;
    }
  }
